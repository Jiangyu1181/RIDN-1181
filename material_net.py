...
class inputMaterial:
    def __init__(self, name, lightPower, lightXPos, lightYPos, lightZPos, camXPos, camYPos, camZPos, uvscale, uoffset,
                 voffset, rotation, identifier, path):
...
parser.add_argument("--input_dir",
                    help="path to xml file, folder or image (defined by --imageFormat) containing information images")
...
parser.add_argument("--testMode", type=str, default="auto", choices=["auto", "xml", "folder", "image"],
                    help="Which loss to use instead of the L1 loss")
parser.add_argument("--imageFormat", type=str, default="png", choices=["jpg", "png", "jpeg", "JPG", "JPEG", "PNG"],
                    help="Which format have the input files")
...
parser.add_argument("--loss", type=str, default="l1",
                    choices=["l1", "specuRough", "render", "flatMean", "l2", "renderL2"],
                    help="Which loss to use instead of the L1 loss")
parser.add_argument("--useLog", dest="useLog", action="store_true", help="Use the log for input")
parser.set_defaults(useLog=False)
parser.add_argument("--logOutputAlbedos", dest="logOutputAlbedos", action="store_true",
                    help="Log the output albedos ? ?")
parser.set_defaults(logOutputAlbedos=False)
parser.add_argument("--l1_weight", type=float, default=100.0, help="weight on L1 term for generator gradient")

parser.add_argument("--nbDiffuseRendering", type=int, default=3,
                    help="Number of diffuse renderings in the rendering loss")
parser.add_argument("--nbSpecularRendering", type=int, default=6,
...
Model = collections.namedtuple("Model",
                               "outputs, gen_loss_L1, gen_grads_and_vars, train, rerendered, gen_loss_L1_exact")
...
def instancenorm(input):
    with tf.variable_scope("instancenorm"):
        ...
        scale = tf.get_variable("scale", [1, 1, 1, channels], dtype=tf.float32,
...
def deconv(batch_input, out_channels):
    with tf.variable_scope("deconv"):
        in_height, in_width, in_channels = [int(batch_input.get_shape()[1]), int(batch_input.get_shape()[2]),
                                            int(batch_input.get_shape()[3])]
        # filter = tf.get_variable("filter", [4, 4, out_channels, in_channels], dtype=tf.float32, initializer=tf.random_normal_initializer(0, 0.02))
        filter = tf.get_variable("filter", [4, 4, in_channels, out_channels], dtype=tf.float32,
                                 initializer=tf.random_normal_initializer(0, 0.02))
        filter1 = tf.get_variable("filter1", [4, 4, out_channels, out_channels], dtype=tf.float32,
                                  initializer=tf.random_normal_initializer(0, 0.02))

        # [batch, in_height, in_width, in_channels], [filter_width, filter_height, out_channels, in_channels]
        #     => [batch, out_height, out_width, out_channels]
        resized_images = tf.image.resize_images(batch_input, [in_height * 2, in_width * 2],
...
def tf_generate_distance(batchSize):
    gaussian = tf.random_normal([batchSize, 1], 0.5, 0.75,
                                dtype=tf.float32)  # parameters chosen empirically to have a nice distance from a -1;1 surface.
    ...
    for elem in tree.findall('.//item'):
        ...
        if not (imagePath is None) and os.path.exists(imagePath):
            ...
            if not (substanceName in exampleDict):
                ...
                pathDict[
                    imagePath] = material  # Add only a path to be queried as for each image we will grab the others that are alike with the other dict
...
def fullyConnected(input, outputDim, useBias, layerName="layer", initMultiplyer=1.0):
    with tf.variable_scope("fully_connected"):
        ...
        weights = tf.get_variable("weight", [inputChannels, outputDim], dtype=tf.float32,
                                  initializer=tf.random_normal_initializer(0, initMultiplyer * tf.sqrt(
                                      1.0 / float(inputChannels))))
        ...
        if (useBias):
            bias = tf.get_variable("bias", [outputDim], dtype=tf.float32,
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    with tf.variable_scope("globalNetwork_fc_1"):
        globalNetwork_fc_1 = fullyConnected(globalNetworkInput, a.ngf * 2, True,
        ...
        with tf.variable_scope("encoder_%d" % (len(layers) + 1)):
            ...
            with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
                nextGlobalInput = tf.concat(
                    [tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis=1), axis=1), mean], axis=-1)
                globalNetwork_fc = ""
                if layerCount + 1 < len(layer_specs) - 1:
                    globalNetwork_fc = fullyConnected(nextGlobalInput, layer_specs[layerCount + 1], True,
                                                      "globalNetworkLayer" + str(len(globalNetworkOutputs) + 1))
                else:
                    globalNetwork_fc = fullyConnected(nextGlobalInput, layer_specs[layerCount], True,
                                                      "globalNetworkLayer" + str(len(globalNetworkOutputs) + 1))
    ...
    with tf.variable_scope("encoder_8"):
        ...
        with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
            ...
            nextGlobalInput = tf.concat(
                [tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis=1), axis=1), mean], axis=-1)
            globalNetwork_fc = fullyConnected(nextGlobalInput, a.ngf * 8 * a.depthFactor, True,
                                              "globalNetworkLayer" + str(len(globalNetworkOutputs) + 1))
            globalNetworkOutputs.append(tf.nn.selu(globalNetwork_fc))
    ...
    for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
        ...
        with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
            ...
            with tf.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):
                nextGlobalInput = tf.concat(
                    [tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis=1), axis=1), mean], axis=-1)
                globalNetwork_fc = fullyConnected(nextGlobalInput, out_channels, True,
                                                  "globalNetworkLayer" + str(len(globalNetworkOutputs) + 1))
                globalNetworkOutputs.append(tf.nn.selu(globalNetwork_fc))
            if dropout > 0.0:
...
def tf_generateSpecularRendering(batchSize, surfaceArray, targets, outputs):
    ...
    currentShift = tf.concat(
        [tf.random_uniform([batchSize, 2], -1.0, 1.0), tf.zeros([batchSize, 1], dtype=tf.float32) + 0.0001], axis=-1)
    ...
    renderedSpecularOutputs = tf_Render(outputs, wi, wo,
                                        includeDiffuse=a.includeDiffuse)  # tf_Render_Optis(outputs,wi,wo, includeDiffuse = a.includeDiffuse)
    return [renderedSpecular, renderedSpecularOutputs]


def DX(x):
    return x[:, :, 1:, :] - x[:, :, :-1,
                            :]  # so this just subtracts the image from a single-pixel shifted version of itself (while cropping out two pixels because we don't know what's outside the image)
...
def loss_grad(x, y, alpha=0.2):
    loss_val = alpha * loss_l1(x,
                               y)  # here alpha is a weighting for the direct pixel value comparison, you can make it surprisingly low, though it's possible that 0.1 might be too low for some problems
...
def create_model(inputs, targets, reuse_bool=False):
    ...
    if a.loss == "render" or a.loss == "renderL2":
        ...
        surfaceArray = tf.concat([XsurfaceArray, YsurfaceArray, tf.zeros([CROP_SIZE, CROP_SIZE, 1], dtype=tf.float32)],
                                 axis=-1)
    ...
    with tf.name_scope("generator_loss"):
        ...
        if a.loss == "l1":
            ...
            DiffuseL1 = tf.abs(tf.log(epsilon + deprocess(targets[0, :, :, 3:6])) - tf.log(
                epsilon + deprocess(outputs[0, :, :, 3:6]))) * a.diffuseLossFactor
            RoughnessL1 = tf.abs(targets[0, :, :, 6:9] - outputs[0, :, :, 6:9]) * a.roughnessLossFactor
            SpecularL1 = tf.abs(tf.log(epsilon + deprocess(targets[0, :, :, 9:12])) - tf.log(
                epsilon + deprocess(outputs[0, :, :, 9:12]))) * a.specularLossFactor
            ...
            DiffuseL1 = tf.square(tf.log(epsilon + deprocess(targets[0, :, :, 3:6])) - tf.log(
                epsilon + deprocess(outputs[0, :, :, 3:6]))) * a.diffuseLossFactor
            RoughnessL1 = tf.square(targets[0, :, :, 6:9] - outputs[0, :, :, 6:9]) * a.roughnessLossFactor
            SpecularL1 = tf.square(tf.log(epsilon + deprocess(targets[0, :, :, 9:12])) - tf.log(
                epsilon + deprocess(outputs[0, :, :, 9:12]))) * a.specularLossFactor
            ...
            with tf.name_scope("renderer"):
                ...
                if a.loss == "render":
                    gen_loss_L1 = tf.reduce_mean(
                        tf.abs(tf.log(rerenderedTargets + 0.01) - tf.log(rerenderedOutputs + 0.01)))
                if a.loss == "renderL2":
                    gen_loss_L1 = tf.reduce_mean(
                        tf.square(tf.log(rerenderedTargets + 0.01) - tf.log(rerenderedOutputs + 0.01)))
        consistencyLoss = 0
                ...
                for step in range(max_steps):
                    ...
                    if should(a.progress_freq):
                        ...
                        print("progress  epoch %d  step %d  image/sec %0.1f" % (
                            train_epoch, train_step, global_step * a.batch_size / (time.time() - start_time)))
...
def tf_Render(svbrdf, wi, wo, includeDiffuse=True):
    ...
    result = result * tf.maximum(0.0, NdotL) / tf.expand_dims(tf.maximum(wiNorm[:, :, :, 2], 0.001),
                                                              axis=-1)  # This division is to compensate for the cosinus distribution of the intensity in the rendering
